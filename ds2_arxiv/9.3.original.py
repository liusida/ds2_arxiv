...
if __name__ == "__main__":
    def main():
        ...
        for i in range(args.num_epochs):
            matrix, indices, vec_detected, vec_swapped = detect_and_swap_gpu(
                matrix, indices, threads_per_block=threads_per_block, blocks=num_blocks, seed=i+args.seed, mode=mode)
            if vec_detected < 1 and mode == 'all':
                break  # finished
            elif vec_detected < 100:  # start enumerate mode
                if num_blocks < 4096:
                    num_blocks *= 2
            if vec_detected < 20 and num_blocks == 4096:
                mode = 'all'
            # else:
            #     mode = 'random'
            p1 = loss_gpu(matrix)
